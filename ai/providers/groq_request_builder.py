# ai/providers/groq_request_builder.py
import base64
import logging
from typing import List, Dict, Any
from ai.models import AIRequest
from .groq_errors import GroqCapabilityError

logger = logging.getLogger("GroqRequestBuilder")

class GroqRequestBuilder:
    """Translates generic application AIRequest metadata payloads into Groq SDK input bodies."""

    @staticmethod
    def build_payload(request: AIRequest, default_model: str) -> Dict[str, Any]:
        """Maps an abstract model request onto valid parameter payloads accepted by Groq endpoints."""
        ...
        if request.image_data:
            if "vision" not in model.lower():
                raise GroqCapabilityError(f"Model '{model}' does not support vision/image inputs. Select a vision-capable model.")
